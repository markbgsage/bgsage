...
"""Continuation: alpha=3.1 (250k) then alpha=1.0 (250k) for player backgame NN.

Picks up from the best weights of the high-LR experiment.

Usage:
    python bgsage/scripts/run_backgame_high_lr_phase2.py
"""
...
def run():
    print(f'=== Backgame High-LR Phase 2: alpha=3.1 then alpha=1.0 ===', flush=True)
    ...
    # Start from best weights of the high-LR experiment
    wpath = f'bgsage/models/{WNAME}.weights'
    best_path = f'bgsage/models/{WNAME}.weights.best'
    if not os.path.exists(best_path):
        print(f'ERROR: {best_path} not found — run high-LR experiment first', flush=True)
        return

    best_er = benchmark_er(bench_boards_raw, bench_eq, best_path)
    gc.collect()
    print(f'Starting ER (from high-LR best): {best_er:.2f}\n', flush=True)
